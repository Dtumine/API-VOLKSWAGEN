const API_BASE = 'http://localhost:4000/api/servicios';
...
async function loadServicios() {
  ...
  try {
    ...
    table.innerHTML = `
      <thead>
        <tr>
          <th>ID</th><th>ID Auto</th><th>ID Empleado</th><th>ID Cliente</th><th>Fecha Servicio</th><th>Descripción</th><th>Costo</th><th>Kilometraje</th><th>Entrada</th><th>Entrega</th><th>Estado</th><th>Acciones</th>
        </tr>
      </thead>
      <tbody></tbody>
    `;
...
async function editServicio(id) {
  ...
  try {
    ...
    if (s.fecha_servicio) {
      ...
      document.getElementById('fecha_servicio').value = iso;
    }
    if (s.fecha_ingreso) {
      const d = new Date(s.fecha_ingreso);
      const iso = d.toISOString().substring(0,10);
      document.getElementById('fecha_ingreso').value = iso;
    }
    if (s.fecha_entrega) {
      const d = new Date(s.fecha_entrega);
      const iso = d.toISOString().substring(0,10);
      document.getElementById('fecha_entrega').value = iso;
    }
